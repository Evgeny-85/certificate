...
describe('main page', () => {
    describe('data rendering', () => {
        // Data was taken from https://js-55fbfg.stackblitz.io/ using files from 'fixtures'
        // (Files were provided with test task)
        const example: {
            fileName: string;            
            subjectCN: string;
            IssuerCN: string;
        ...
        example.forEach(file => {
            it(`${file.fileName} may be uploaded`, () => {
                // Generate file path, do it could be read.
                const filePath = `cypress/fixtures/${file.fileName}`
                // Read file data.
                cy.readFile(filePath)
                // Imitate 'drag-n-drop' in the UI.
            ...
            // Check for each row in the UI that it matches testing data.
    ...
    describe('certificate upload', () => {
        ...
        describe('check if data is shown correctly', () => {
            before(() => {
                ...
                cy.visit('https://js-55fbfg.stackblitz.io/')
                // Make sure site is fully loaded.
                cy.wait(500)
            })
            it('no data is shown', () => {
                // No certificates were uploaded yet.
